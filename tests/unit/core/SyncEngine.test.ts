...
describe('SyncEngine', () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(path.join(projectRoot, 'AGENTS.md'), '# Local AGENTS\n');
  });
  ...
  it('syncs preset-mode sources, skips source skiller.toml, and reports removed lock entries', async () => {
    const presetRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), 'skiller-preset-'),
    );
    try {
      await writeFile(
...
`,
      ...
      await writeFile(path.join(presetRoot, 'AGENTS.md'), '# Preset AGENTS\n');
      await writeFile(
      ...
      await writeFile(
        path.join(presetRoot, '.claude', 'prompt.yml'),
        'hook: 1\n',
      );
      await writeFile(
      ...
      await writeFile(path.join(projectRoot, '.agents', 'old.md'), 'stale\n');
      await writeFile(
      ...
      expect(firstResult.synced).not.toContain('.agents/AGENTS.md');
      ...
      await expect(
        fs.stat(path.join(projectRoot, 'package.json')),
      ).rejects.toThrow();
...
`,
      ...
      await expect(
        fs.readFile(
          path.join(projectRoot, '.agents', 'rules', 'task.mdc'),
          'utf8',
        ),
