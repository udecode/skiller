...
describe('Sync lifecycle CLI', () => {
  it('syncs preset sources before install and prunes outputs removed from synced skills-lock.json on update', () => {
    const projectRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), 'skiller-sync-install-'),
    );
    const presetRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), 'skiller-preset-root-'),
    );
    const skillSourceRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), 'skiller-skill-source-'),
    );
    ...
    writeFile(path.join(projectRoot, 'AGENTS.md'), '# Local\n');
    writeFile(
...
`,
    ...
    writeFile(path.join(presetRoot, 'AGENTS.md'), '# Preset\n');
    writeFile(
  ...
  it('supports repo-mode sync-only with include and exclude rules', () => {
    const projectRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), 'skiller-sync-repo-'),
    );
    const sourceRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), 'skiller-sync-repo-src-'),
    );
...
`,
    ...
    writeFile(
      path.join(sourceRoot, '.agents', 'rules', 'task.mdc'),
      '# task\n',
    );
    writeFile(
    ...
    writeFile(
      path.join(sourceRoot, '.claude', 'scripts', 'hook.sh'),
      'echo hi\n',
    );
  ...
  it('runs sync across nested project roots', () => {
    const projectRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), 'skiller-sync-nested-'),
    );
    const presetRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), 'skiller-sync-nested-preset-'),
    );
    ...
    writeFile(path.join(projectRoot, 'AGENTS.md'), '# Root\n');
    writeFile(
      path.join(projectRoot, '.agents', 'skiller.toml'),
      `[sync]
source = ${JSON.stringify(presetRoot)}
`,
    ...
    writeFile(
      path.join(projectRoot, 'templates', 'child', 'AGENTS.md'),
      '# Child\n',
    );
    writeFile(
